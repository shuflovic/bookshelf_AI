...
const App: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<AppView>('settings'); // Always start on settings
  ...
  useEffect(() => {
    try {
      ...
      if (storedSettings) {
        ...
        // Remove the automatic redirect logic - let users manually navigate
      } else {
        setSettings({});
        // Already on settings view by default
      }
    } catch (e) {
  ...
  const handleSaveSettings = (newSettings: AppSettings) => {
    ...
    // After saving settings, move to upload view
